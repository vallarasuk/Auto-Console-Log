...
class CSharpProvider extends LogProvider {
  ...
  addOperation(document, selection, varName, insertLine, logOperations, scheduled, declarationLine) {
    ...
    if (this.shouldSkipVariable(varName) || CSHARP_KEYWORDS.has(varName) || resolvedInsertLine >= document.lineCount) return;
    ...
    if (this.hasNearbyLog(document, resolvedInsertLine, varName, ["Console.Write"])) return;
